...
class RephineLayer(nn.Module):
    def __init__(
        ...
        # reduce_tuples=False
            self,
            n_features,  # 输入节点特征维度
            n_filtrations,  # 过滤函数数量
            filtration_hidden,  # 过滤函数网络的隐藏层维度
            out_dim,  # 输出拓扑特征维度
            diagram_type="rephine",  # 持久图类型（"rephine"或"standard"）
            dim1=True,  # 是否计算1维同调特征
            sig_filtrations=True,  # 是否对过滤函数输出使用Sigmoid
            reduce_tuples=False  # 是否简化持久图元组
    ):
        super().__init__()
#激活函数
        final_filtration_activation = nn.Sigmoid() if sig_filtrations else nn.Identity()
#持久同调计算  自定义的rephine方法和standard方法
        self.persistence_fn = (
